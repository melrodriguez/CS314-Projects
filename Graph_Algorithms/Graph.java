 ...
 *
 *  On my honor, <Melody>, this programming assignment is my own work
...
public class Graph {
    ...
    public void dijkstra(String startName) {
        ...
        Vertex current = getVertex(startName);

        // Set everything to 0 since it is start
        current.numEdgesFromStartVertex = 0;
        current.weightedCostFromStartVertex = 0;
        priorityQueue.add(new Path(current, 0));

        while (!priorityQueue.isEmpty()) {
            Path currentPath = priorityQueue.remove();
            current = currentPath.dest;
            ...
            if (current.scratch == 0) {
                ...
                for (Edge e : current.adjacent) {
                    ...
                    double sum = current.weightedCostFromStartVertex + e.cost;
                    if (sum < destination.weightedCostFromStartVertex) {
                        // Vertex is not visited so we add it
                        destination.weightedCostFromStartVertex = sum;
                        destination.prev = current;
                        destination.numEdgesFromStartVertex = current.numEdgesFromStartVertex + 1;
                        priorityQueue.add(new Path(destination, destination.weightedCostFromStartVertex));
                    }
                }
            }
        }
    ...
    public void findAllPaths(boolean weighted) {
        ...
        for (String vertexName : vertices.keySet()) {
            Vertex current = getVertex(vertexName);
            current.clearPathInfo();
            clearAll();
            // Use dijkstra to find all shortest weighted to each vertex
            if (weighted) {
                dijkstra(vertexName);
            }
            // Unweighted path use this method
            else {
                findUnweightedShortestPath(vertexName);
            }

            for (Vertex v : vertices.values()) {
                // If not connected then the weighted cost shall be infinity
                boolean isConnected = v.weightedCostFromStartVertex != INFINITY;
                // Find a connected vertex
                if (isConnected && !v.name.equals(vertexName)) {
                    ...
                    if (getPath(v.name).compareTo(longest) > 0) {
