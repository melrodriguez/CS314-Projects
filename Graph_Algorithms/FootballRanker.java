...
public class FootballRanker {
    ...
    private double printRootMeanSquareError(List<String> humanRanks,
        ...
        ArrayList<String> ranksPredicted = new ArrayList<>();

        // Add the treeset information to the list
        for (AllPathsInfo info: paths) {
            ranksPredicted.add(info.getName());
        }

        for (int i = 0; i < humanRanks.size(); i++) {
            // Actual data
            String humanRank = humanRanks.get(i);
            result += Math.pow((i + 1) - (ranksPredicted.indexOf(humanRank) + 1), 2);
            if (showResults) {
                System.out.printf("%-20s - actual rank: %d predicted rank: %d%n", humanRank, (i + 1),
                        ranksPredicted.indexOf(humanRank) + 1);
            }
        }
        result = Math.sqrt(result / humanRanks.size());
        if (showResults) {
            System.out.printf("Root Mean Square Error: %.4f", result);
        }

        // Make result to the nearest tenth
